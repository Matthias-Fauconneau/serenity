...
struct System {
    ...
    static constexpr float dt = 1./ 2000;
    ...
    struct Friction {
        ...
        vec3 localA, localB; // Last static friction contact location in relative reference frames
        size_t lastUpdate = 0; // Time step of last update (to know whether to update static friction locations)
        vec3 normal = 0;
        float fN = 0;
    ...
    struct Grain {
        ...
        //static constexpr float density = 100* M / cb(L);
        static constexpr float mass = 3e-3; //density * volume;
        ...
        static constexpr float angularMass = 2./5*mass*(pow5(radius)-pow5(radius-1e-4))
    ...
    struct Wire {
        ...
        static constexpr float elasticModulus = 1e5 * M / (L*T*T); // 1e6
        static constexpr float normalDamping = 1 * M / T; // 0.3
        static constexpr float frictionCoefficient = 1;
    ...
    template<Type tA, Type tB> void friction(Friction& f, tA& A, size_t a, tB& B, size_t b) {
        vec3 fT = 0;
        ...
        if(tangentRelativeSpeed < staticFrictionThresholdSpeed) {
            ...
            if(tangentLength) {
                constexpr float staticFrictionStiffness = /*max(frictionCoefficient / min(tA::radius, tB::radius),
                                                                                            1*Grain::elasticModulus*Grain::radius);*/
                        10 * 20000;
                //log(staticFrictionStiffness);
...
struct Grid {
    ...
    Grid(float scale, vec3 size) : scale(scale), size(scale*size), indices(size.z*size.y*size.x*cellCapacity)
     { indices.clear(); }
    struct List : mref<uint16> {
        ...
        void append(uint16 index) { // Sorted by decreasing index
            ...
            else assert_(j==cellCapacity);
        }
    };
    inline List operator[](size_t i) { return indices.slice(i*cellCapacity, cellCapacity); }
    size_t index(int x, int y, int z) {
        return (z*size[1]+y)*size[0]+x;
    }
    int3 index3(vec3 p) { // [-size/2·R..size/2·R, 0..size·R] -> [0..size-1]
        return size/2+int3(scale*p);
    }
    size_t index(vec3 p) {
        int3 i = index3(p);
...
struct Simulation : System {
    // Space partition
    //Grid grainGrid {2./Grain::radius, 16*Grain::radius};
    //, wireGrid {2./Grain::radius}; // FIXME: too large wire grid for efficiency
    ...
    Time wireContactTime, wireFrictionTime, wireBoundTime, wireTensionTime, wireIntegrationTime;
    ...
    void step() {
        ...
        {grainTime.start();
            ...
            parallel_for(grain.count, [this](uint, int a) {
                /*if(0) {
                    ...
                    for(int b: range(grain.count)) if(a!=b) penalty(grain, a, grain, b);
                }
            });
                ...
                for(size_t i=0; i<grain.frictions[a].size;) {
                    ...
                    if(f.lastUpdate >= timeStep-1)  {
                        if(f.lastUpdate == timeStep)  {
                            size_t b = f.index;
                            if(b==floor.base) friction(f, grain, a, floor, 0);
                            else if(b==side.base) friction(f, grain, a, side, 0);
                            else friction(f, grain, a, grain, b-grain.base);
                        } // else no more contact or dynamic friction
                        i++;
                    }
                    else { grain.frictions[a].removeAt(i); continue; } // Garbage collection
                }
            });
        ...
        {wireTime.start();
            ...
            parallel_for(wire.count, [this](uint, int a) {
                ...
                /*if(0) { // Wire - Grain
                    ...
                    for(int b: range(grain.count)) if(a!=b) penalty(wire, a, grain, b);
                }
            });
            ...
            parallel_for(wire.count, [this](uint, size_t a) {
                for(size_t i=0; i<wire.frictions[a].size;) {
                    ...
                    if(f.lastUpdate >= timeStep-1)  {
                        if(f.lastUpdate == timeStep)  {
                            size_t b = f.index;
                            if(b==floor.base) friction(f, wire, a, floor, 0);
                            else if(b==side.base) friction(f, wire, a, side, 0);
                            else friction(f, wire, a, grain, b-grain.base);
                        } // else no more contact or dynamic friction
                        i++;
                    }
                    else { wire.frictions[a].removeAt(i); continue; } // Garbage collection
                }
            });
        ...
        if(rollTest) {
            ...
            Locker lock(this->lock);
            /*plots[0].dataSets["Ecc"__][timeStep] = 1./2*Grain::mass*sq(grain.velocity[0]);
            plots[0].dataSets["Ecr"__][timeStep] = 1./2*Grain::angularMass*sq(grain.angularVelocity[0]);
...
struct SimulationView : Simulation, Widget, Poll {
    ...
    shared<Graphics> graphics(vec2 size) {
        renderTime.start();
        glDepthTest(true);
        const float scale = 1 / (16*Grain::radius);
        mat4 viewProjection = mat4() .scale(vec3(scale, scale*size.x/size.y, -1))
        ...
        if(grain.count-start) {
            ...
            for(size_t i: range(grainCount)) {
                ...
                float3 min = O - float3(float2(scale*Grain::radius), 0); // Isometric
                float3 max = O + float3(float2(scale*Grain::radius), 0); // Isometric
                positions[i*6+0] = min;
            ...
            GLBuffer rotationBuffer (apply(grain.rotation.slice(start),
                                           [=](quat q) -> quat { return q.conjugate()*viewRotation.conjugate(); }));
            shader.bind("rotationBuffer"_, rotationBuffer);
            shader["radius"] = Grain::radius / 2;

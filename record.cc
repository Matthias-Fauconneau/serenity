...
void Record::start(const ref<byte>& name, bool video, bool audio) {
    av_register_all();
    if(context) stop();
...
void Record::capture(float* audio, uint audioSize) {
    if(videoStream && videoTime*audioCodec->sample_rate <= audioStream->pts.val*videoStream->time_base.den) captureVideoFrame();

...
static inline void report(uint64 totalTime, const Scene::Timers timers[], const uint bounce, Scene::Path path, const uint offset, const uint stride) {
    if(bounce > 1) return;
...
struct Render {
    ...
    void clear() { mcast<Float>(map).clear(0); iterations=0; }
    void step() {

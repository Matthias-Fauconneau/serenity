...
buffer<byte> inflate(ref<byte> compressed, buffer<byte>&& target) { //inflate huffman encoded data
    ...
    for(;;) {
        ...
        if(BTYPE==0) { // Literal
            ...
            if(BTYPE==1) { // Static Huffman codes
                mref<uint>(literal.count, 9).copy({0,0,0,0, 0,0,24,24+144,256-144});
                ...
                for(int i: range(256-144)) literal.lookup[24+144+8+i] = 144+i; // 144-255: 9
                mref<uint>(distance.count, 6).copy({0,0,0,0,0,32});
            ...
            for(;;) {
                ...
                if(symbol < 256) target[targetIndex++] = symbol; // Literal
                    ...
                    if(symbol==285) length = 258;
                        ...
                        for(int i: range(6)) if(symbol < code[i+1]) { length = lengths[i]+((symbol-code[i])<<i)+(i?bitIO.read(i):0); break; }
                    }
                    ...
                    for(size_t i : range(length)) {
                        assert_(targetIndex+i < target.size, BTYPE, BFINAL, symbol, length, targetIndex, targetIndex+i, target.size, targetIndex-i+length);
                        target[targetIndex+i] = target[targetIndex-distance+i]; // length may be larger than distance
                    }
                    targetIndex += length;
                } else error(symbol);
            }
    ...
    target.size = targetIndex;
    //assert_(target.size == targetSize, targetSize, target.size, target.capacity);
    assert_(target.size >= targetSize && target.size <= target.capacity, targetSize, target.size, target.capacity);
    return move(target);
}

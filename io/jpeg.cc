...
Image decodeJPEG(const ref<byte> file) {
  ...
  jpeg_mem_src(&jpeg, reinterpret_cast<const uint8*>(file.data), file.size);
  ...
  for(int y : range(image.size.y)) {
      uint8* line = (uint8*)(image.begin()+y*image.size.x);
...
buffer<byte> encodeJPEG(const Image& image, int quality) {
  ...
  jpeg_mem_dest(&jpeg, &(uint8*&)const_cast<byte*&>(file.data), &file.size);

...
inline void read(BinaryData& s, String& output) { s.align(4); uint size=s.read(); output = copyRef(s.Data::read(size)); s.advance(1);/*0*/ }
...
template<> inline void sign<int>(String& s) { s.append('i'); }
template<> inline void sign<uint>(String& s) { s.append('u'); }

template<> inline void sign<string>(String& s) { s.append('s'); }
template<> inline void sign<String>(String& s) { s.append('s'); }

template<> inline void sign<ObjectPath>(String& s) { s.append('o'); }

//template<class T> void sign< Signature<T> >(String& s) { s.append('g'); } //function template partial specialization is not allowed

//template<class T> void sign< variant<T> >(String& s) { s.append('v'); sign<T>(s); } //function template partial specialization is not allowed
template<> inline void sign< variant<String> >(String& s) { s.append('v'); }
template<> inline void sign< variant<int> >(String& s) { s.append('v'); }

//template<Type K, Type V> inline void sign<DictEntry<K,V>>(String& s) { s.append('{'); sign<K>(s); sign<V>(s); s.append("}"); } //function template partial specialization is not allowed
template<> inline void sign<DictEntry<String,variant<String>>>(String& s) { s.append('{'); sign<String>(s); sign<variant<String>>(s); s.append('}'); }

//template<Type T> inline void sign<array<T>>(String& s) { s.append('a'); sign<T>(s); } //function template partial specialization is not allowed
template<> inline void sign<array<DictEntry<String,variant<String>>>>(String& s) { s.append('a'); sign<DictEntry<String,variant<String>>>(s); }
...
template<class... Args> void serialize(array<byte>& s, Signature<Args...>) {
    String signature; sign<Args...>(signature); assert(signature); s.append((uint8)signature.size); s.append(move(signature)); s.append(0);
}
...
template<class T> void serialize(array<byte>& s, const array<T>& array) { serialize(s, (ref<T>)array); }

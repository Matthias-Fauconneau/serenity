...
template<class T> size_t DataStream<T>::available(size_t need) {
 while((int64)need>(int64)Data::available(need) && T::poll(1000)) {
  ::buffer<byte> chunk = T::readUpTo(max(256*1024ll, int64(need)-(int64)Data::available(need)));
...
// Cache
const Folder& cache() { static Folder cache(".cache", home()); return cache; }
...
String str(const URL& url) {
 return url.scheme+(url.scheme?"://"_:""_)+url.authorization+(url.authorization?"@"_:""_)+url.host+url.path+(url.fragment?"#"_:""_)+url.fragment+
   (url.post?"?"_:""_)+url.post;
   //(url.post?(string)url.post:""_);
}
...
void HTTP::request() {
 String request = (url.post?"POST"_:"GET"_)+" "_+(startsWith(url.path,"/"_)?""_:"/"_)+url.path+" HTTP/1.1\r\nHost: "_+url.host+"\r\n"
   ;//"User-Agent: Serenity\r\n"_;
 array<String> headers = copy(this->headers);
 if(url.post) {
...
void HTTP::header() {
 ...
 while(!match("\r\n"_)) {
  ...
  if(key=="Content-Length"_) contentLength=parseInteger(value);
   ...
   url.post = copyRef(this->url.post);
   if(url.scheme != this->url.scheme || url.host != this->url.host) {
    ...
    this->url = ::move(url);
    disconnect();
    connect(ip, this->url.scheme=="https"_?443:80, this->url.scheme=="https"_);
    Poll::fd = SSLSocket::fd;
   }
   redirectCount++; assert_(redirectCount < 2);
   data = {};
   index = 0;
   buffer = array<byte>();
   contentLength=0;
...
void HTTP::receiveContent() {
 ...
 if(contentLength) {
  ...
  //assert(available(3) >= 3, available(3));
  while(available(3)>=3) {
   if(chunkSize==0) { chunkSize = integer(false, 16); skip("\r\n"_); } //else already parsed
   if(chunkSize==0) { state=Cache; break; }
   if(available(chunkSize+5)<uint(chunkSize+5)) { return; }
   ref<byte> chunkChunk = Data::read(::min(chunkSize, available(chunkSize)));
   content.append(chunkChunk);
   chunkSize -= chunkChunk.size;
   assert_(chunkSize == 0);
   if(chunkSize==0 && available(2)>=2) skip("\r\n"_);
  }
 }
 else error("Missing content", chunked, contentLength, buffer);
#else // Progressive cache
 if(chunked) {
  do {
   ...
   if(chunkSize) { // Packet breaks within chunk
    ...
    chunkSize -= size;
    log(size, chunkSize, available(chunkSize));
...
void HTTP::cache() {
 ...
 if(content.size>64*1024) log("Downloaded",url.path,content.size/1024,"KB"); else log("Downloaded",url.path,content.size,"B");
...
void HTTP::event() {
 ...
 if(!(revents&POLLIN) && state <= Content) { log("No data",url,revents,available(chunkSize),Data::available(chunkSize),chunkSize); /*done();*/ /*return;*/ }
 if(state == Header) { header(); }
 if(state == Content) { assert_(revents==POLLIN && available(max(3ul, chunkSize))>=3); receiveContent(); }
 if(state == Cache) {
  cache();
...
Map getURL(URL&& url, function<void(const URL&, Map&&)> contentAvailable, int maximumAge, HTTP::State wait) {
 ...
 if(wait == HTTP::Available) {
  ...
  while(request.state < wait) {
   if(request.state > HTTP::Request) assert_(request.events==POLLIN, request.events, (int)request.state);
   if(0) request.event(); /*Busy wait*/ else assert_(request.wait(), request.events, request.revents);
  }

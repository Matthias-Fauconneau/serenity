...
void log_(const Item& item) { log_(item.rule.symbol,"→ ");log_(slice(item.rule.tokens, 0, item.dot)," ");log_(" •");log_(slice(item.rule.tokens, item.dot)," "); }

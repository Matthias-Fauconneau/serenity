...
void PDF::render(vec2 scale, vec2 offset) {
	...
	offset.y += scroll/size.y;

	blit.bind(); blit["scale"] = scale; blit["offset"] = offset; //blit["color"] = vec4(0,0,0,1);
	//glHard();
...
bool PDF::event(int2, int event, int state) {
	if(event==WheelDown && state==Pressed) { scroll-=size.y/4; needUpdate.emit(); return true; }
	if(event==WheelUp && state==Pressed) { scroll+=size.y/4; needUpdate.emit();return true; }
	return false;
}

...
array<SourceImage> ProcessedImageGroupSource::images(size_t groupIndex, int outputIndex, int2 size, bool noCacheWrite) {
	if(!size) size=this->size(groupIndex);
...
array<SourceImage> ProcessedGroupImageGroupSource::images(size_t groupIndex, int outputIndex, int2 size, bool noCacheWrite) {
	assert_(operation.outputs() == 1);
	assert_(outputIndex == 0);
	if(!size) size=this->size(groupIndex);

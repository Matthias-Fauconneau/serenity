...
SART::SART(int3 size, const ImageArray& b) : Reconstruction(size, b), Ai(b.size), Ati(size), Ax(b.size), h(b.size), L(size), oldX(size), LoAti(size) {
...
void SART::step() {
    ...
    LoAti = CLVolume(x.size);
    update(LoAti, LoAti, lambda, L, Ati); // x = | x + λ L / Ati |+
    copy(oldX, x);

...
CL(operators, divdiff) inline uint64 divdiff(const ImageArray& y, const ImageArray& a, const ImageArray& b, const ImageArray& c) { return emulateWriteTo3DImage(CL::divdiff, y, noneNearestSampler, a, b, c); } // y = ( a - b ) / c [SIRT]
CL(operators, maxadd) inline uint64 maxadd(const CLVolume& y, const CLVolume& a, const float alpha, const CLVolume& b) { return emulateWriteTo3DImage(CL::maxadd, y, noneNearestSampler, a, alpha, b); } // y = max(0, a + α b) [SIRT, CG]
...
CL(operators, muldiv) inline uint64 muldiv(const CLVolume& y, const CLVolume& a, const CLVolume& b, const CLVolume& c) { return emulateWriteTo3DImage(CL::muldiv, y, noneNearestSampler, a, b, c); } // y = max(0, a + c ? a * b / c : 0) [MLTR]

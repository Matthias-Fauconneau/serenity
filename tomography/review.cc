...
struct Application : Widget {
    ...
    map<array<string>, String> points;
    ...
    Application() {
        for(string name: names) {
            ...
            String data = readFile(name, results);
            data = copy(String(section(data,' ',3,3)));
            points.insert(move(arguments), move(data));
        }
        log(values);
        window.setSize(-1);
        window.show();
    ...
    void render() override {
            ...
            for(uint level: range(parameters[axis].size)) {
                ...
                for(uint x: range(X)) {
                    ...
                    for(uint index: range(size)) {
                        ...
                        //Text(str(origin)).render(clip(target, origin*cellSize+Rect(cellSize)));
                    }
                }
                X *= size;
            }
        ...
        for(uint X: range(cellCount(0))) {
            for(uint Y: range(cellCount(1))) {
                ...
                Text(points.contains(key)?points.at(key):""_).render(clip(target, ((levelCount().yx()+int2(1)+int2(X,Y)) * cellSize)+Rect(cellSize)));
            }
        }
#endif
    }

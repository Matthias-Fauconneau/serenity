...
inline float sum(const VolumeF& A) {
    ...
    setKernelArgs(sumKernel, buffer, output, elementCount);
    clCheck( clSetKernelArg(sumKernel, 3, threadCount*sizeof(float), 0) );
    size_t localSize = threadCount;
    size_t globalSize = blockCount * localSize;
    clCheck( clEnqueueNDRangeKernel(queue, sumKernel, 1, 0, &globalSize, &localSize, 0, 0, 0), "SSQ");

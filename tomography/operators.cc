...
CL(sum, sum) float sum(const CLVolume& A, const int3 origin, int3 size) {
    size = size?:A.size;
    if(isIntel) return sum(A.read(VolumeF(size,A.name), origin)); // reduce fails on Intel
    return reduce1(CL::sum, A, origin, size);
}
CL(sum, SSQ) float SSQ(const CLVolume& A, const int3 origin, int3 size) {
    size = size?:A.size;
    if(isIntel) return SSQ(A.read(VolumeF(size,A.name), origin)); // reduce fails on Intel
...
CL(sum, dotProduct)  float dotProduct(const CLVolume& A, const CLVolume& B, const int3 origin, int3 size) {
    size = size?:A.size;
    if(isIntel) return dotProduct(A.read(VolumeF(size,A.name), origin), B.read(VolumeF(size,B.name), origin)); // reduce fails on Intel

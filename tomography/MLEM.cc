...
void MLEM::initialize(const ref<Projection>& projections, const ref<ImageF>& images) {
        ...
        for(uint unused i: range(size)) {
            ...
            if(sq(origin.xy()) < radiusSq) {
                ...
                for(uint projectionIndex: range(projections.size)) {
                    ...
                    v4sf start,end;
                    if(intersect(projection, xy, volume, start, end)) {
                ...
                if(sum) xData[z*X*Y + y*X + x] *= sum / count;
            }
        }
    });
    time.stop();

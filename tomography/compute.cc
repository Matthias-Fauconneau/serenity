...
struct Application : Poll {
    ...
    Application(string path) : Poll(0,0,thread), projectionData(loadCDF(path)), referenceData(loadCDF(path)), projections(evaluateProjections(volumeSize, projectionData.size.xy(), projectionCount, stride, true)) { /*queue();*/ thread.spawn(); }
    void event() {

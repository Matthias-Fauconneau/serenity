...
VolumeF loadCDF(const Folder& folder) {
    ...
    for(string name: files) {
        ...
        return VolumeF(size, cast<float>((ref<byte>)Map(file)));
    }
    // Generates memory map-able (little endian) data
    ...
    log("Generated");
    return VolumeF(int3(X,Y,Z), data);
}

...
struct Convert {
 Convert() {
  ...
  mat3 m = camRGB.inverse() * mat3(whiteBalance*(0xFFF/float(max-min)));
  constexpr int shift = 31-12-3; float scale = 1<<shift;
  for(float c: m.data) assert_(c < 1<<3);
  ...
  for(size_t Y: range(size.y)) {
   for(size_t X: range(size.x)) {
    ...
    int32 cR = r-min;
    int32 cG = g1+g2-2*min;
    int32 cB = b-min;
    int R = ::min((m00 * cR + m01 * cG + m02 * cB)>>shift, 0xFFF);
    int G = ::min((m10 * cR + m11 * cG + m12 * cB)>>shift, 0xFFF);
    int B = ::min((m20 * cR + m21 * cG + m22 * cB)>>shift, 0xFFF);

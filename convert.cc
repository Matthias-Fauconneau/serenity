...
struct Convert {
 Convert() {
  ...
  for(float c: RGB_sensor.data) assert_(c < 1<<7);
  ...
  for(size_t tY: range(size.y)) {
   for(size_t tX: range(size.x)) {
    ...
    assert_(cR>=0 && cR<1<<15 && cG>=-1 && cG<1<<15 && cB>=-1 && cB<1<<15, cR, cG, cB, tX, tY, x, y);
    ...
    assert_(r>=-(1<<15) && r<1<<15 && g>=-(1<<15) && g<1<<15 && b>=-(1<<15) && b<1<<15, r, g, b);
    assert_(Y >= -8 && Y < 1<<15, Y);
    histogram[::max(0,Y)]++;
    R(tX, tY) = r;
    G(tX, tY) = g;
    B(tX, tY) = b;

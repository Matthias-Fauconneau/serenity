...
/// Deinterleaves Bayer cells (for better prediction (compression))
buffer<uint16> deinterleave(const Image16& source) {
	...
	for(size_t y: range(Y/2)) for(size_t x: range(X/2)) {
		G[(y*2+0)*X/2+x] = source[(y*2+0)*X+(x*2+0)];
		R[(y*1+0)*X/2+x] = source[(y*2+0)*X+(x*2+1)];
		B[(y*1+0)*X/2+x] = source[(y*2+1)*X+(x*2+0)];
		G[(y*2+1)*X/2+x] = source[(y*2+1)*X+(x*2+1)];
	}
	return target;
}

/// Reinterleaves Bayer cells (for compatibility)
Image16 interleave(ref<uint16> source, int2 size) {
	Image16 target(size);
	...
	for(size_t y: range(Y/2)) for(size_t x: range(X/2)) {
		target[(y*2+0)*X+(x*2+0)] = G[(y*2+0)*X/2+x];
		target[(y*2+0)*X+(x*2+1)] = R[(y*1+0)*X/2+x];
		target[(y*2+1)*X+(x*2+0)] = B[(y*1+0)*X/2+x];
		target[(y*2+1)*X+(x*2+1)] = G[(y*2+1)*X/2+x];
	}
	return target;
}
...
struct Compress {
	Compress() {
		for(string fileName: currentWorkingDirectory().list(Files/*|Recursive*/)) {
			...
			buffer<byte> encoded = Encoder(source.size).write(deinterleave(source)).end();

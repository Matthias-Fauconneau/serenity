...
void Horizontal::update() {
	int width = size.x-2*margin, expanding=0;
	for(Widget* child : *this) {
		int2 size=child->sizeHint();
		if(size.x) width -= size.x; else expanding++;
		width -= margin*2;
...
Text::Text(string&& text, int fontSize) : font(Font::instance(fontSize)), text(move(text)) {};
...
void List::render(vec2 scale, vec2 offset) {
	...
	glQuad(flat,vec2(at(current)->position-int2(margin,margin)),
		   vec2(at(current)->position+at(current)->size+int2(margin,margin)));
}

/// Button

Button::Button(const Image& icon,bool toggle) : enable(new GLTexture(icon)), toggle(toggle) {}
Button::Button(const Image& enable, const Image& disable)
: enable(new GLTexture(enable)), disable(new GLTexture(disable)), toggle(false) {}

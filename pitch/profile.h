...
struct OffsetPlot : Widget {
    ...
    void render(RenderTarget2D& target, vec2 unused offset=0, vec2 size=0) override {
        ...
        for(int key: range(keyCount)) {
            ...
            target.fill(vec2(x0, min(y0,y1)), vec2(dx, abs(y0-y1)), sign*p1>0 ? red : blue, 1.f);
            ...
            target.fill(vec2(x0, min(y1,y2)), vec2(dx, abs(y1-y2)), 3.f/4*(sign*p2>0 ? red : blue), 1.f);
            ...
            target.fill(vec2(x0, min(y2,y3)), vec2(dx, abs(y2-y3)), 1.f/2*(sign*p3>0 ? red : blue), 1.f);
            ...
            target.fill(vec2(x0, min(y4,y0)), vec2(dx, abs(y4-y0)), 1.f/2*(sign*p4>0 ? red : blue), 1.f);
        }
    }
};

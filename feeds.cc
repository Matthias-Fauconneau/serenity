...
struct Feeds : Application {
    ...
    void loadFeeds(const array<string>& feeds) {
        for(const string& url: feeds) getURL(url,false); //pipeline
        for(const string& url: feeds) {
            ...
            for(Link& item: items) {
                if(item.url && !isRead(item)) getURL(item.url,false); //prefetch entries

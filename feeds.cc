...
Feeds::Feeds() : readConfig("read"_,config(),ReadWrite|Create|Append), readMap(readConfig) {
    array::reserve(64);
    activeChanged.connect(this,&Feeds::setRead);
    itemPressed.connect(this,&Feeds::readEntry);
    load();
}
...
void Feeds::loadFeed(const URL& url unused, Map&& document) {
    ...
    assert(channel.host);
    Favicon* favicon=0;
    for(unique<Favicon>& f: favicons) if(f->host==channel.host) { favicon=&f; break; }
    if(!favicon) {
        assert(channel.host);
        favicons << unique<Favicon>(move(channel.host));
        favicon = &favicons.last();
        favicon->imageChanged.connect(&listChanged,&signal<>::operator());
    }

    array<unique<Entry>> entries; int count=0;
    auto addEntry = [this,&favicon,&count,&entries](const Element& e)->void{
        ...
        if(!isRead(guid, link)) entries<< unique<Entry>(move(guid),move(link),favicon,move(title)); //display all unread entries
        else if(count==0) entries<< unique<Entry>(move(guid),move(link),favicon,move(title),12); //display last read entry
        count++;
    };
    feed.xpath("feed/entry"_,addEntry);
    feed.xpath("rss/channel/item"_,addEntry);
    for(int i=entries.size-1;i>=0;i--) this->entries << move(entries[i]), *this << &this->entries.last(); //display oldest first
    listChanged();
}
...
void Feeds::readEntry(uint index) {
    ...
    pageChanged(entry.link, toUTF8(entry.text.text), entry.icon.image);
    if(index+1<count()-1) getURL(URL(entries[index+1]->link)); //preload next entry (TODO: preload image)
}

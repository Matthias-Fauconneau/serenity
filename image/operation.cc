...
SourceImage ImageOperation::image(size_t imageIndex, size_t componentIndex, int2 hint, string parameters) {
	...
	if(source.outputs() == operation.inputs() || (operation.inputs()==0 && operation.outputs()==0)) {
		return move(::cacheGroup<ImageF>(path(), elementName(imageIndex)+parameters, this->size(imageIndex, hint), outputs(), time(imageIndex),
            ...
            //for(const auto& e: inputs) assert_(e.stride%8==0);
            //for(const auto& e: outputs) assert_(e.stride%8==0);
            log(toString());
	...
	return ::cache<ImageF>(path(), elementName(imageIndex)+'['+str(componentIndex)+']', this->size(imageIndex, hint),
                                                                                 time(imageIndex), [&](const ImageF& output) {
...
SourceImage ImageGroupFold::image(size_t groupIndex, size_t componentIndex, int2 hint, string parameters) {
	return move(::cacheGroup<ImageF>(path(), elementName(groupIndex), size(groupIndex, hint), outputs(), time(groupIndex),
		...
		if(operation.inputs()==1 || operation.inputs()==0) { // Forwards componentIndex
            for(size_t componentIndex: range(this->outputs())) {
                ...
                //for(const auto& e: inputs) assert_(e.stride%8==0);
                //for(const auto& e: outputs.slice(componentIndex, 1)) assert_(e.stride%8==0);
                log(toString());
                operation.apply(outputs.slice(componentIndex, 1), unsafeRef(inputs));
                log("OK");
			}
		} else {
...
array<SourceImage> ImageGroupOperation::images(size_t groupIndex, size_t componentIndex, int2 hint, string parameters) {
	if(outputs() > 1 && source.outputs() == operation.inputs() && operation.outputs() == outputs()) {
		Folder folder = ::cacheFolder(path(), elementName(groupIndex)+parameters, strx(size(groupIndex, hint)), time(groupIndex),
            ...
            //for(size_t inputIndex: range(source.outputs())) for(const auto& e: images[inputIndex]) assert_(e.stride%8==0, e.stride, groupIndex, "Input");
            for(size_t imageIndex: range(groupSize(groupIndex))) { // Operates on each image separately
				::cacheGroup<ImageF>(folder, str(imageIndex), size(groupIndex, hint), outputs(), time(groupIndex),
                    ...
                    //for(const auto& e: inputs) assert_(e.stride%8==0, operation.name(), e.stride, groupIndex, imageIndex, "Input");
                    //for(const auto& e: outputs) assert_(e.stride%8==0, operation.name(), groupIndex, imageIndex, "Output");
                    Time time{true};
		...
		return ::apply(groupSize(groupIndex), [&folder, size=this->size(groupIndex, hint), componentIndex](size_t imageIndex) {
			Map map(str(componentIndex), Folder(str(imageIndex)+'.'+strx(size), folder));
                        ImageF image (unsafeRef(cast<typename ImageF::type>(map)), size/*, align(8, size.x)*/);

...
struct Document {
    const String source;
    string formatString;
	Format format = formatString == "A4"_ ? A4() : Format{vec2(1024,768), vec2(64,0), "DejaVuSans"_, 0, 24, 24, 32, 1};
    ...
    Text& newText(Page& page, string text, float size, bool center=true) const {
		return element<Text>(page, text, size, 0, 1, (format.pageSize - 2.f*format.marginPx).x, format.font, false, interlineStretch, center);
    }
    ...
    Widget* parseImage(TextData& s, Page& page, bool quick) const {
		...
		return &element<ImageView>(page, share(page.images.last()));
    }
    ...
    Page parsePage(TextData& s, Header& currentHeader, uint pageIndex, bool quick=false) const {
                ...
                if(command == "tableofcontents"_) {
                    ...
                    for(const Header& header: headers) {
						...
						if(format.footerSize)
							grid.append(&newText(page, " "_+str(header.page), format.textSize)); // Show page numbers
                    }
					page.append(&grid);
                } else error(command);
                continue;
		...
		if(format.footerSize && page.index>=3) page.footer = &newText(page, str(page.index), format.textSize);
        return move(page);
    }
...
struct DocumentViewer {
	...
	Window window {&view, int2(document.format.pageSize), [&]{ return str(view.pageIndex); }};

    Page getPage(int pageIndex) {
        if(window) {
            window.render();
			window.setTitle(str(pageIndex));
			writeFile(lastPageIndexPath, str(pageIndex));
        }

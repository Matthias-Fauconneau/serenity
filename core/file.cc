...
buffer<byte> Stream::read(size_t size) {
 ...
 size_t offset=0;
 while(offset<size) {
     size_t increment = check(::read(fd, buffer.begin()+offset, size-offset));
     assert_(increment);
     offset += increment;
 }
 assert(offset==size);
...
buffer<byte> File::readAll() { return read(size()); }
...
int64 writeFile(const string path, const ref<byte> content, const Folder& at, bool overwrite) {
 assert_(overwrite || !existsFile(path, at), path);
...
void copy(const Folder& oldAt, const string oldName, const Folder& newAt, const string newName, bool overwrite) {
 ...
 for(off64_t offset=0, size=oldFile.size(); offset<size;)
  offset += check(sendfile64(newFile.fd, oldFile.fd, &offset, size-offset), (int)newFile.fd, (int)oldFile.fd, offset, size-offset, size);
...
string environmentVariable(const string name, string value) {
 static auto environ = File("/proc/self/environ").readUpTo(16384);
 for(TextData s(environ);s;) {
  string key = s.until('=');
  string value = s.until('\0');

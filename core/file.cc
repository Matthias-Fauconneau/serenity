...
void Stream::read(mref<byte> target) {
 ...
 /*size_t offset=0; while(offset<target.size) {
  int size = check(::read(fd, target.begin()+offset, target.size-offset));
  if(!size) log(size);
  offset += size;
 }*/
}
...
int Device::ioctl(uint request, void* arguments, int pass) {
    ...
    if(-*__errno_location()==pass) { /*__errno_location()=0;*/ return status; }

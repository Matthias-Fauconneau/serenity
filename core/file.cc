...
// File
File::File(const string& path, const Folder& at, Flags flags):Stream(check(openat(at.fd, strz(path), flags, 0666),/*at.name()+"/"_+*/path, (int)flags, path.size)){ assert_(path.size<0x100); }

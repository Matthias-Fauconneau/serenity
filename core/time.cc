...
Date parseDate(TextData& s) {
 ...
 for(;;) {
  s.whileAny(" ,\t");
  for(int i=0;i<12;i++) {
   constexpr string en[12] = {"january","february","march","april","may","june","july","august","september","october","november","december"};
   constexpr string de[12] = {"januar","februar","märz","april","mai","juni","juli","august","september","oktober","november","dezember"};
   constexpr string De[12] = {"Januar","Februar","März","April","Mai","Juni","Juli","August","September","Oktober","November","Dezember"};
   if(s.match(months[i]) || s.match(en[i]) || s.match(de[i]) || s.match(De[i]) || s.match(months[i].slice(0,3)) || s.match(De[i].slice(0,3))) {
    date.month=i;
  ...
  if(s.isInteger()) {
   ...
   else if(date.year==-1 && number>2000) {
    //assert_(number>31||number==16, number, s);
    date.year=number;
    //if(date.year<100) date.year+=2000;
   }
   else if((date.year==-1||date.month==-1||date.day==-1) && s.match("-")) {
    assert_(date.year==-1, date.year, date.month, s.data);
    if(number > 2000) {
     date.year=number;
     if(date.month==-1) {
      date.month=s.integer()-1;
     ...
     date.day=number-1;
     if(s.isInteger()) {
      assert_(date.month==-1, s, date, s.untilEnd());
      date.month=s.integer()-1;
      if(!s.match("-")) return Date();
      //assert_(s.match("-"), s);
      assert_(date.year==-1);
      date.year=s.integer();
     }
   ...
   else if(date.day==-1 && number <= 31) {
    date.day=number-1;
    if(number%10==1) s.match("st");
   }
   //else if(date.month==-1 && number <= 12) date.month=number-1;
   else return {}; //error("Invalid date", s);
  }
  else if(s.match("Anfang ")) date.month = 0;
  else if(s.match("Mitte ")) date.day = 14;
  else if(s.match("Ende ")) date.day = 30;
  else break;
continue2_:;
 }

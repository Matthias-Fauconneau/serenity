...
struct Map : mref<byte> {
 ...
 explicit Map(const string path, const Folder& at=currentWorkingDirectory(), Prot prot=Read, Flags flags=Shared, ::Flags fileFlags=ReadOnly) : Map(File(path,at,fileFlags), prot, flags) {}

...
#define check(expr, message...) ({ long e=(long)expr; if(e<0 && -e<LAST) error(#expr ""_, linuxErrors[*__errno_location()], ##message); e; })
/// Aborts if \a expr is negative and logs corresponding error code (unused result)
#define check_(expr, message...) ({ long unused e=expr; if(e<0 && -e<LAST) error(#expr ""_, linuxErrors[*__errno_location()], ##message); })
/// Does not abort if \a expr is negative and logs corresponding error code (unused result)
#define check__(expr, message...) ({ long unused e=expr; if(e<0 && -e<LAST) log(#expr ""_, linuxErrors[*__errno_location()], ##message); })

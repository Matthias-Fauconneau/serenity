...
generic struct array : buffer<T> {
 ...
 void reserve(size_t nextCapacity) {
  ...
  if(nextCapacity>capacity) {
   ...
   if(capacity) {
    ...
    const T* data = 0;
    if(posix_memalign((void**)&data,32,nextCapacity*sizeof(T))) error("Out of memory"_); // TODO: move compatible realloc
    swap(data, this->data);
 ...
 template<Type F> size_t add(F&& e) { size_t i = ref<T>::indexOf(e); if(i!=invalid) return i; else { append(forward<F>(e)); return size-1; } }
 ...
 size_t addSorted(const T& e) { size_t index=reverseLinearSearch(e); if(index==0 || at(index-1) != e) insertAt(index, ::copy(e)); return index; }

...
static Lock threadsLock __attribute((init_priority(104)));
...
Thread mainThread __attribute((init_priority(104))) (0);
...
static Semaphore results __attribute((init_priority(102)));
...
static thread workers[threadCount] __attribute((init_priority(102)));
...
__attribute((constructor(103))) static inline void spawnWorkers() {
    assert_(threadCount);
    string OMP_NUM_THREADS = environmentVariable("OMP_NUM_THREADS");
...
void parallel(function<void(uint)> delegate) {
    for(uint index: range(threadCount)) workers[index].delegate = &delegate;
    jobs.release(threadCount);
...
// ::argc = argc, ::argv = argv;
 ...
 requestTermination(groupExitStatus);
...
int execute(const string path, const ref<string> args, bool wait, const Folder& workingDirectory, Handle* stdout, Handle* stderr,
 ...
 for(string env: envs) env0.append( env ); // FIXME: Remove duplicates (override)

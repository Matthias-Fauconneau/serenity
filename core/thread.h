...
template<Type F> void parallel_chunk(size_t jobCount, F f) {
 ...
 parallel([&](uint i) { f(i, i*chunkSize, min<size_t>(chunkSize, jobCount-i*chunkSize)); }, chunkCount);
}
...
int execute(const string path, const ref<string> args={}, bool wait=true, const Folder& workingDirectory=currentWorkingDirectory(), Handle* stdout = 0, Handle* stderr = 0, const ref<string> envs={});

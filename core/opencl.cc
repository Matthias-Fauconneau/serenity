...
CLMem::~CLMem() { if(pointer) { clReleaseMemObject(pointer); handleCount--; } pointer=0; }

CLRawBuffer::CLRawBuffer(size_t size, string name) : CLMem(clCreateBuffer(context, CL_MEM_READ_WRITE, size, 0, 0), name) {}

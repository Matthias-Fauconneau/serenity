...
struct Editor : Widget {
    ...
    struct Cone { vec3 A,nA; vec3 B,nB; };
    array<Cone> cones;
    ...
    void generate() {
        ...
        for(const Module& module : system.generate(level)) { char symbol=module.symbol;
            ...
            else if(symbol==']') previousState = state = stack.pop();
            else if(symbol=='f' || symbol=='F') {
                float l = module.arguments[0]?:1, w1 = module.arguments[1]?:1, w2 = module.arguments[2]?:1;
                vec3 A = previousState[3].xyz();
                vec3 nA = previousState.normalMatrix()*vec3(0,w1,0);
                state.translate(vec3(l,0,0)); //forward axis is +X
                vec3 B = state[3].xyz();
                vec3 nB = state.normalMatrix()*vec3(0,w2,0);
                if(symbol=='F') cones << Cone __(A,nA,B,nB);
                previousState=state;
    ...
    void render(int2 targetPosition, int2 targetSize) override {
        ...
        for(Cone cone: cones) {
            // project end points
            vec4 A=view*cone.A; vec3 nA = scale*/*normalMatrix**/cone.nA; float wA=length(nA);
            vec4 B=view*cone.B; vec3 nB = scale*/*normalMatrix**/cone.nB; float wB=length(nB);
            vec3 X = normalize(B.xyz()-A.xyz()), Y=cross(vec3(0,0,1),X), Z=cross(X,Y);
            // compute line equation to interpolate [-1,1] across cone
            float a=-X.y, b=X.x, c= a*A.x + b*A.y;
            float l = sqrt(a*a+b*b)*max(wA,wB) * 1.5 /*bias to avoid invalid square root (result in an NPR flatten effect)*/; a/=l, b/=l; c/=l;

            function<vec4(float,float)> shader = [/*uniform*/sky,sun,/*TODO: face attribute*/ Y,Z, /*TODO: varying*/a,b,c](float x, float y){
                float d = a*x+b*y-c;
                assert(d>=-1 && d<=1, d);
            ...
            raster.line(A,B,wA,wB,shader); //TODO: split shader from raster pass
        }

...
struct LSystem : Widget {
    ...
    LSystem() {
        ...
        window.localShortcut(Key(' ')).connect([this]{max=min(8,max++); window.render();});
    }
    void render(int2 position, int2 size) override {
        window.setTitle(dec(max));
        array<vec2> polygon; polygon
                << vec2(cos(1*PI/6),sin(1*PI/6))
                << vec2(cos(5*PI/6),sin(5*PI/6))
                << vec2(cos(9*PI/6),sin(9*PI/6))
                << vec2(cos(1*PI/6),sin(1*PI/6));
        array<vec2> generator; generator << vec2(0,0) << vec2(1./3,0) << vec2(1./3+cos(PI/3)/3,-sin(PI/3)/3) << vec2(2./3,0) << vec2(1,0);

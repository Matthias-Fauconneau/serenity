...
LayoutRender::LayoutRender(Layout&& _this, const float _mmPx, const float _inchPx) : Layout(move(_this)) {
	...
	buffer<v4sf> elementColors = apply(sources.size, [&](const size_t elementIndex) {
		...
		if(arguments.contains("hue"_)) { H = parse<float>(arguments.at("hue"_)) * 0xFF; log(H); }
...
#define px(x) round((x)*mmPx)
	...
	// -- Renders elements
	buffer<Image4f> renders = apply(elements, [=](Element& e) { return e.render(mmPx); });

	auto compose = [&]{
		Image4f target = copy(background);
	...
	if(arguments.value("blur","1/8"_)!="0"_) {
		...
		Image4f transpose(int2(size.y, size.x));

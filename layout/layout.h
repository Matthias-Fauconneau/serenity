...
struct Element {
	...
	virtual ~Element() {}
	virtual Image4f render(float mmPx) abstract;
	virtual Image source() const abstract;
};
...
struct Table {
	...
	inline ref<Cell> row(size_t y) const { assert(y<size_t(size.y)); return cells.slice(y*size.x, size.x); }
	inline Cell& operator()(size_t x, size_t y) const { assert(x<size_t(size.x) && y<size_t(size.y), x, y); return cells[y*size.x+x]; }
	inline Cell& operator()(int2 index) const { return operator()(index.x, index.y); }
};

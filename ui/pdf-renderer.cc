   ...
   if(resources.contains("XObject"_)) {
    ...
    for(auto e: dict) {
     ...
     if(depth==8 && object.dict.contains("ColorSpace"_)) {
      Variant cs = object.dict.at("ColorSpace"_).type == Variant::Integer ? parseVariant(xref[object.dict.at("ColorSpace"_).integer()]) :
        ::move(object.dict.at("ColorSpace"_));
      if(cs.data=="DeviceGray"_ ) {}
      else if(cs.data=="DeviceRGB"_ || (cs.list && cs.list[0].data=="CalRGB"_)) { if(depth==8) depth=24; }
      else {
       if(cs.list[0].data=="Indexed"_ && cs.list[1].data=="DeviceGray"_ && cs.list[2].integer()==255) {
        ...
        indexed=true;
       } else { log("Unsupported colorspace",cs,cs.list[1].type==Variant::Integer?parseVariant(xref[cs.list[1].integer()]).data:""_); continue; }
      }
     }
     const uint8* src = (uint8*)object.data.data;
     //assert_(object.data.size>=image.height*((image.width*depth+7)/8), object.dict, object.data.size, image.height*image.width*depth/8, depth);
     ...
     } else if(depth==24) for(uint y=0;y<image.height;y++) for(uint x=0;x<image.width;x++,dst++,src+=3) {
      if(size_t(src-(uint8*)object.data.data) >= object.data.size) { log(object.dict); break; }
      dst[0]=byte4(src[0],src[1],src[2],0xFF);
     }
     else if(depth==32) {

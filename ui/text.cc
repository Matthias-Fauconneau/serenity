    ...
    if(words) {
        ...
        for(const ref<Glyph> word: words) {
            ...
            for(Glyph glyph: word) {
                ...
                //bbMax.y = ::max(bbMax.y, glyph.origin.y-glyph.bearing.y+glyph.size.y);
                bbMax.x = ::max(bbMax.x, glyph.origin.x-glyph.bearing.x+glyph.advance);
...
TextLayout::TextLayout(const ref<uint> text, float size, float wrap, string fontName, bool hint, float interline, int align,
    ...
    for(auto& line: lines) {
        ...
        this->lines.append( ::Line{vec2(minX, midY), vec2(maxX, midY), 0} );
    }
}
...
void Text::render(RenderTarget2D& target, vec2 offset, vec2 size) {
    ...
    for(const auto& line: layout.glyphs) for(const auto& word: line) for(Glyph e: word) { e.origin += offset; target.glyph(e.origin, e.fontSize, e.font, e.code, e.index, e.color, e.opacity); }
    for(auto e: layout.lines) { e.p0 += offset; e.p1 +=offset; target.line(e.p0, e.p1, e.color, e.opacity, e.hint); }
}

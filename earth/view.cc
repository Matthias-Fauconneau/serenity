...
struct Terrain {
	...
	buffer<GLWorker> glWorkers {1, 0}; // Decode and uploads tiles

	Terrain(Window& window) : contentChanged({&window, &Window::render}) {
		glWorkers.append(window, function<void()>{this,&Terrain::work});
	}

	void queue() { if(loadQueue) for(GLWorker& worker: glWorkers) worker./*Poll::*/post(); }
	void work() {

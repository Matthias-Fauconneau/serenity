...
void AudioFile::read(int16* output, uint outputSize) {
    ...
    for(;;) {
        ...
        for(;;) { //fill input buffer
            ...
            if( file->streams[packet.stream_index]==audioStream ) {
                ...
                if(audio->sample_fmt == AV_SAMPLE_FMT_FLT) {
                    input = (float*)frame.data[0]; inputBufferSize=0;
                } else {
                    input = allocate<float>(inputBufferSize=inputSize*audioInput.channels);
        ...
        if(resampler) {
            ...
            if(audio->sample_fmt != AV_SAMPLE_FMT_FLT) unallocate(input,inputBufferSize); //else owned by avcodec
            input = buffer; inputBufferSize=bufferSize; bufferSize=0;
            inputSize = out;
        }
    }
EndOfFile:

    ...
    long processModule(const ref<byte>& target) {
        ...
        if(compile) {
            ...
            if(!existsFile(object, folder) || lastCompileEdit >= File(object).modifiedTime()) {
                ...
                /*if(::find(build,"assert"_)) args << string("-g"_) << string("-Og"_) << string("-DNO_INLINE"_) << string("-DASSERT"_);
                else*/ if(::find(build,"debug"_)) args << string("-g"_) << string("-Og"_) << string("-DNO_INLINE"_) << string("-DASSERT"_);

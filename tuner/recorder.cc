...
#define CAPTURE 0
#if CAPTURE
    AudioInput input{{this,&Recorder::write}, rate, N}; //CA0110 driver doesn't work
    ...
    uint write(int16* output, uint size) {
        ...
        if(k > rate/keyToPitch(21+88+1) && k < rate/keyToPitch(21-1)) {
            ...
            if(abs(offset) < 1 && key==lastKey && previousEnergies[0]<previousEnergies[1] && previousEnergies[1]>e) {
                ...
                int velocity = round(0x100*sqrt(previousEnergies[1]));
                int maxVelocity = 0;
                ...
                log(str(rate/k)+" Hz\t"_+strKey(max(0,key+21))+" \t"_+str(100*offset)+" \t+/-"_+str(100*error)+" cents\t"_+str(velocity));
            }
        }
        lastKey = key;

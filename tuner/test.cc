...
struct Plot : Widget {
    ...
    void render(int2 position, int2 size) {
        ...
        for(uint i : range(estimator.candidates.size)) {
            ...
            String text = strKey(pitchToKey(96000*candidate.f0/estimator.N))+"."_+dec(round(candidate.B ? 1./candidate.B : 0))
        ...
        }
        {
            ...
            {float x = this->x(50.*N/rate+0.5)*size.x;
                fill(position.x+x-2,position.y,position.x+x+2,position.y+size.y, vec4(1,0,0,1./2)); }
            {float x = this->x(3*50.*N/rate+0.5)*size.x;
                fill(position.x+x-2,position.y,position.x+x+2,position.y+size.y, vec4(1,1,0,1./4)); }
            {float x = this->x(5*50.*N/rate+0.5)*size.x;
                fill(position.x+x-2,position.y,position.x+x+2,position.y+size.y, vec4(1,1,1,1./8)); }
        }
    }
};
...
struct PitchEstimation {
    ...
    void next() {
        ...
        for(; t<=stereo.size/2-N; t+=periodSize) {
            ...
            const float harmonicThreshold = 2./3; //3; //1; //1./2 //4
            const float ratioThreshold = 0.17; //1./6; //1./5
            ...
            if( f0 > fMin
                ...
                if(expectedKey==key) {
                    ...
                    spectrum.iMin = min(50*N/rate, (uint)min(f0, expectedF));
                    ...
                    if(ratio<1./2 &&
                            (
                                ((offsetF0>1./5 || expectedKey<=parseKey("G1"_)) && key==expectedKey-1)
                             ...
                             || ((previousKey==expectedKey || previousKey==expectedKey-1 ||
                                 ...
                                 && expectedKey<=parseKey("F0"_))
                             || (t%(5*rate)<2*rate && previousKey==expectedKey && ratio<1./3 /*&& key==expectedKey-12*/))) {
                        if(0) {}
                        else if(offsetF0>1./3 && key==expectedKey-1 && apply(split("A2 A#1 A1 F1 E1 D#1 D1 C#1 C1 B0 A#0 G0 F#0 E0"_), parseKey).contains(expectedKey)) {
                            log("-"_); lastKey=expectedKey; // Avoid false negative from mistune
                        }
                        else if(offsetF0>1./4 && key==expectedKey-1 && apply(split("D4 G#1 F#1 D#1 C#1 C1 A#0"_), parseKey).contains(expectedKey)) log("-"_);
                        else if(offsetF0>1./5 && key==expectedKey-1 && apply(split("B1 A#1 G1 D#1 D1"_), parseKey).contains(expectedKey)) log("-"_);
                        else if(offsetF0>1./6 && key==expectedKey-1 && apply(split("C1 B0"_), parseKey).contains(expectedKey)) log("-"_);
                        else if(key==expectedKey-1 && apply(split("D#1 C#1 C1 A#0"_), parseKey).contains(expectedKey)) log("-"_);
                    ...
                    } else { log("FIXME",ratio<1./2, key==expectedKey-1, expectedKey<=parseKey("D#0"_) ); break; }
                }
                tries++;

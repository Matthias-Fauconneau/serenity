...
int2 display() {
    ...
    if(!display) {
        ...
        int size = fScreen.stride * display.y; assert(fScreen.stride>=display.x*sizeof(pixel),fScreen.stride,fScreen.type,display.x,sizeof(pixel),vScreen.bits_per_pixel,fScreen.id);
        pixel* fb = (pixel*)mmap(0, size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0); assert(fb);

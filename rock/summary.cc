...
class(Summary, Tool) {
    ...
    void execute(const Dict& arguments, const ref<Result*>& outputs, const ref<Result*>&, Process& process) override {
        ...
        {Plot otsu; otsu.title = String("Interclass deviation versus threshold"_), otsu.xlabel=String("μ"_), otsu.ylabel=String("σ"_);
            otsu.legends << String("Radiodensity probability"_); otsu.dataSets << parseNonUniformSample(process.getResult("distribution-radiodensity"_, args)->data);
        ...
        properties << Text("Porosity: "_+format(Bold)+dec(round(parseScalar(process.getResult("porosity"_, args)->data)*100))+"%"_);
        real resolution = parseScalar(process.getResult("resolution"_, args)->data);
        for(auto result : map<string, string>({"critical-radius"_,"mean-radius"_,"representative-radius"_},{"Critical bottleneck radius"_,"Mean pore radius"_,"Representative radius"_})) {
            real length = parseScalar(process.getResult(result.key, args)->data);

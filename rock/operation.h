...
struct Result : shareable {
    Result(const string& name, long timestamp, Dict&& localArguments, String&& metadata, buffer<byte>&& data)
        : name(name), timestamp(timestamp), localArguments(move(localArguments)), metadata(move(metadata)), data(move(data)) {}
    ...
    Dict localArguments; /// Relevant arguments (defined only by process and user)
...
template<> inline String str(const Result& o) { return o.name+str(o.localArguments); }

...
void glViewport(int2 position, int2 size) { glViewport(position.x,position.y,size.x,size.y); }
void glClear() { glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT); }
...
GLShader::GLShader(const ref<byte> &vertex, const ref<byte> &fragment) {
    ...
    {uint shader=glCreateShader(GL_VERTEX_SHADER);
        glShaderBinary(1, &shader, binaryFormats[0], vertex.data, vertex.size);
        glAttachShader(id, shader);
    }
    {uint shader=glCreateShader(GL_FRAGMENT_SHADER);
        glShaderBinary(1, &shader, binaryFormats[0], fragment.data, fragment.size);
        glAttachShader(id, shader);
    }
...
uint GLShader::attribLocation(const char* name) {
    ...
    assert(location>=0,"Unknown attribute"_,name);

...
inline string str(const Matrix& a) {
    string s; s<<'[';
    for(int i=0;i<a.m;i++) {
        if(a.n==1) s = s+"\t"_+str(a(i,0));
        else {
            for(int j=0;j<a.n;j++) {
                s = s+"\t"_+str(a(i,j));
            }
...
/// Transposition (reflects over main diagonal)
...
inline string str(const Vector& a) { return str((const Matrix&)a); }
...
Vector solve(const Matrix& A, const Vector& b) {
    ...
    // Solves Ax=b <=> AtAx=Atb using Cholesky decomposition (A=LL*), forward substitution (Lx = b) and backward substitution (Ux = y)

...
struct Isometric : Widget {
    ...
    Isometric() {
        ...
        for(LASPoint p: cast<LASPoint>(map.slice(header.data)).slice(0, header.count)) {
            ...
            short3 position((p.position-minI)*(N-1)/(maxI.x-minI.x));
            ...
            byte4 color = image(position.x*(4096-1)/(N-1), 4096-1-position.y*(4096-1)/(N-1));
    ...
    shared<Graphics> graphics(vec2 unused size) {
        ...
        for(Point p: points) {
            ...
            //int z8 = 0xFF*((mean.z+z)-min.z)/(max.z-min.z);
            if(X >= 0 && Y>= 0 && Y < H && X < W) /*if(z8>target(X, Y)[0])*/ target(X, Y) = p.color;
        }

...
template<class T> array<T> reverse(array<T>&& a) { for(uint i=0; i<a.size()/2; i++) swap(a[i], a[a.size()-i-1]); return move(a); }
...
Expression abs(const Expression& a) {
    if(a.type == Integer) return Expression(abs(a.integer)); //integer
...
string Expression::str() const {
    ...
    if(type==Add) return replace(string(A+"+"_+B),"+-"_,"-"_);
    if(type==Mul)  {
        if(a->type==Pow && a->b->type==Integer && a->b->integer==-1) return B+A.slice(1);
        if(b->type==Integer && a->type!=Integer) return move(B<<A);
        return A+"*"_+B;
    }
    if(type==Pow) {
        if(b->type == Integer && b->integer==-1) return "1/"_+A;
        const ref<byte> superscript[] = {"⁰"_,"¹"_,"²"_,"³"_,"⁴"_,"⁵"_,"⁶"_,"⁷"_,"⁸"_,"⁹"_,"¹⁰"_,"¹¹"_};
...
float Expression::toFloat() {
    ...
    if(type == Pow) {
        ...
        return pow(a->toFloat(),b->toFloat());
    }
    error(*this);
}
...
void Expression::reduce() { //TODO: generated parser
    ...
    if(type==Integer||type==Symbol) {}
        ...
        if(a->type==Add && b->type==Add) *this= new(new(move(a->a) + move(a->b)) + move(b->a)) + move(b->b);  //left associative (a+b)+(c+d) = ((a+b)+c)+d
            ...
            *this= b->integer-a->a->integer;
            ...
            if(needReduce) {
                for(Expression* op: reverse(move(ops))) op->reduce();
                reduce();
            }
    ...
    debug(
            ...
            if(ulp(check,toFloat())>134) { log(::str(check), before); log(::str(toFloat()),*this); log("ULP distance"_,ulp(check,toFloat())); error(*this); }

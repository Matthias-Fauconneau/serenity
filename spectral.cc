...
struct Spectral {
    /// Parameters
    static constexpr uint n = 16/*0*/; // Node count
    const real dt = 1./n; // Time step (~ Courant–Friedrichs–Lewy condition)
    const real nu = 1; // Diffusion coefficient
    const real H = 1./(nu*dt); // Helmholtz coefficient
    const real w = 2*PI / 30; // Velocity source frequency
    ...
    Matrix Q, iQ; // Eigenvector matrix (and inverse)
    ...
    Spectral() {
        ...
        Matrix T(n);
        for(uint i: range(n)) {
            real ci = (i==0||i==n-1) ? 2 : 1;
            for(uint j: range(n)) {
                real cj = (j==0||j==n-1) ? 2 : 1;
        ...
        L(0, 0) = 1; L(n-1,n-1) = 1; // Boundary conditions
        iLb = inverse(Matrix({{L(0,0), L(0,n-1)}, {L(n-1,0), L(n-1,n-1)}})); // Inverse boundary operator
        // Boundary operators
        ...
        const real lowestReliableEigenvalue = min(1., min(apply(eigenvalues, abs<real>)));
...
#define VALIDATION 1
    ...
    void step() {
        ...
        b[0] = -1/(2*nu)*(sin(w*(t-1))+sin(w*t)); // Boundary condition: v(x=0,t) = sin(wt)
        b[n-1] = 0; // Boundary condition: v(x=1,t) = 0
#else
        const real k = 2*PI; // Validation solution wavelength
        Vector v0(n); for(uint i: range(n)) v0[i] = cos(k*x[i]); // Expected solution
        Vector b = (-sq(k) - H)*v0; // Lu = b
        b[0] = v0[0], b[n-1] = v0[n-1]; // Boundary conditions
#endif
        Vector bi = b(1, n-1) - boundaryColumns * (iLb * Vector(b[0], b[n-1])); // Interior constant term
        Vector vi = Q * (eigenvalueReciprocals * (iQ * bi)); // Interior solution
        ...
        v[n-1] = vb[1];
#if VALIDATION
        log("maximumError: 2^"_+str(int(log2(max(apply(v-v0, abs<real>))))));
        exit();
#else
        pNL = move(NL);
        t++;

...
struct mat3 {
    ...
    mat3 operator*(mat3 b) const {
        mat3 r(0); for(int j=0;j<3;j++) for(int i=0;i<3;i++) for(int k=0;k<3;k++) r.m(i,j) += m(i,k)*b.m(k,j); return r;
    }
    ...
    mat3 adjugate(const float idet=1) const { //transpose of cofactors
        ...
        adj(2,1) = -(m(0,0) * m(2,1) - m(2,0) * m(0,1)) * idet;
        adj(2,2) =  (m(0,0) * m(1,1) - m(0,1) * m(1,0)) * idet;
        return adj;
    }
    mat3 inverse() const { return adjugate(1 / det()); }
};
...
struct mat4 {
    ...
    mat4 inverse() const {
        float idet= 1 / (m(0,0) * det3(1, 2, 3, 1, 2, 3) - m(0,1) * det3(0, 2, 3, 1, 2, 3)+

...
#define check(expr, message...) ({ long e=expr; debug( if(e<0 && -e<58) error(#expr##_, string(errno[-e]), ##message); ) e; })
#define check_(expr, message...) ({ long e=expr; debug( if(e<0 && -e<58) error(#expr##_, string(errno[-e]), ##message); ); })

...
void Keyboard::render(const Image& target) {
    ...
    for(uint key=0; key<88; key++) {
        vec3 white = midi.contains(key+21)?red:input.contains(key+21)?blue: ::white;
        int x0 = margin + key*dx;
        int x1 = x0 + dx;
        line(target, int2(x0, 0), int2(x0, y1-1), black);

        int notch[12] = { 3, 1, 4, 0, 1, 2, 1, 4, 0, 1, 2, 1 };
        int l = notch[key%12], r = notch[(key+1)%12];
        if(key==0) l=0; //A-1 has no left notch
        if(l==1) { // black key
            line(target, int2(x0, y1-1), int2(x1, y1-1), black);
            fill(target, Rect(int2(x0+1,0), int2(x1+1,y1-1)), midi.contains(key+21)?red:input.contains(key+21)?blue: ::black);
        } else {
            fill(target, Rect(int2(x0+1,0), int2(x1,y2)), white); // white key
            line(target, int2(x0-l*dx/6, y1-1), int2(x0-l*dx/6, y2), black); //left edge
            fill(target, Rect(int2(x0+1-l*dx/6,y1), int2(x1,y2)), white); //left notch
            if(key!=87) fill(target, Rect(int2(x1,y1), int2(x1-1+(6-r)*dx/6,y2)), white); //right notch
            //right edge will be next left edge
        }
        if(key==87) { //C7 has no right notch
            line(target, int2(x1+dx/2, 0), int2(x1+dx/2, y2), black);
            fill(target, Rect(int2(x1, 0), int2(x1+dx/2, y1-1)), white);
        }
    }
}

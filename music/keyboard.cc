...
void Keyboard::render(const Image& target) {
 ...
 Image light (target.size); // Separate lighting for correct shadow composition (FIXME: ImageF)
 light.clear(0xFF);
 for(uint key=0; key<88; key++) {
  ...
  fill(target, int2(x0, l==1?y0b:y0w), int2(1, y1b-1-(l==1?y0b:y0w)), black); // Vertical edge top part
  ...
  const int S = 4;
  if(l!=1) gradientY(light, x0+1, dx, y0w, S, 1.f/2, 0, 1, Sub); // Top shadow
  if(key==0) l=0; //A-1 has no left notch
  if(l==1) { // black key
   fill(target, int2(x0+1, y0b), int2(1, y1br-1-y0b), color+bgr3f(1.f/8)); // Left vertical edge (highlight)
   fill(target, int2(x0+2, y0b), int2(dx, y1br-1-y0b), color); // Key
   if(!pressed) fill(target, int2(x0+1, y1br-1), int2(dx, 1), color+bgr3f(1.f/8)); // Bottom edge (highlight)
   fill(target, int2(x0+1, y1br), int2(dx, y1b-1-y1br), color+bgr3f(1.f/32)); // Bottom side
   fill(target, int2(x0+1, y1b-1), int2(dx-1, 1), color); // Bottom edge (edge)
  } else { // White key
   fill(target, int2(x0+1, y0w), int2(dx-1, y1b-y0w), color); // White key top
   if(r) fill(target, int2(x0+dx, y1b-1), int2(1, 1), color); // Corner with black
   fill(target, int2(x0-l*dx/6, y1b-1), int2(1, y1w-(y1b-1)), black); // Vertical edge black to bottom
   int left = l*dx/6;
   ...
   if(left) gradientY(light, wx0, x0-wx0, y1b, S, 1.f/2, 0, 1, Sub); // Left Black/White H shadow
   if(!pressed) {
   ...
   if(l) {
    gradientX(light, x0, S, y0w, y1b-y0w, 1.f/2, 0, 1, Sub); // Black|White vertical shadow
    //fill(target, int2(x0, y1b-1), int2(1, 1), color); // Corner with black
   }
 ...
 for(uint i: range(light.ref::size)) mul(target[i], linear(light[i]));
}
#else

...
MidiFile::MidiFile(const ref<byte>& data) { /// parse MIDI header
    ...
    s.advance(10);
    uint16 nofChunks = s.read(); ticksPerBeat = s.read(); log(ticksPerBeat/41, ticksPerBeat/(ticksPerBeat/41));
    ...
    while(uint(track.time*uint64(48*60000/120)/ticksPerBeat) < time) {
        ...
        if((type==NoteOff || type==NoteOn) && active.contains(key)) {
            ...
            if(state==Sort) {
                ...
                if(abs(int(note.time-nearest))<=ticksPerBeat/11) note.time=nearest; // Merges dates under an eighth note at 120 (~62ms, ~16Hz)

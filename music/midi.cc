...
MidiFile::MidiFile(ref<byte> file) { /// parse MIDI header
	...
	divisions = notes.ticksPerSeconds; // Defaults to 60 qpm
...
void MidiFile::read(Track& track, uint trackIndex) {
	...
	while(s) {
		...
		if(type==NoteOn) {
			...
			Accidental noteAccidental = Accidental(Flat+accidentals[keySignature.fifths+1][key%12]);
			signs.insertSorted(Sign{track.time, 0, staff, Sign::Note, .note={
										clef, noteStep, noteAccidental, Quarter /*FIXME: from duration*/, Note::NoTie,
										/*dot*/false, /*grace*/ false, /*slash*/ false, /*staccato*/ false, /*tenuto*/ false, /*accent*/ false, /*trill*/ false, /*up*/false,
										key, invalid, invalid}});
		}

...
generic struct cat<ref<T>, ref<T>, T> {
	...
	operator buffer<T>() const { buffer<T> target (size(), 0); copy(target); return target; }
};
generic cat<ref<T>,ref<T>, T> operator+(ref<T> a, ref<T> b) { return {a,b}; }
ref<char> str(const char* source) { size_t size=0; while(source[size]) size++; return ref<char>(source,size); }
...
inline constexpr ref<char> operator "" _(const char* data, size_t size) { return ref<char>(data,size); }
void log_(ref<char> message) { write(2, message.data, message.size); }

int main(int argc, const char* args[]) {
	if(argc < 1) { log_("Usage:\tflic name.raw16 name.eg6\n\tflic name.eg6 name.raw16\n"_); return -1; }
	...
	buffer<byte> target  = argc < 2 ? str(args[1])+".eg6\0"_ : unsafeRef(str(args[2]));
	int targetFD = open(target.data, 1/*O_WRONLY*/|0100/*O_CREAT*/|01000/*O_TRUNC*/, 0666);
	...
	if(file.size == Y*X*2+256) { // Encode
		...
		if(interleave(decode(target, Y*X), X, Y) != source) return -2;

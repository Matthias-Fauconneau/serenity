...
struct AudioOutput : Device, Poll {
    ...
    AudioOutput(int sampleBits, int rate, int periodSize, Thread& thread);
    ...
    /// \note -1 means maximum
    AudioOutput(function<uint(int16* output, uint size)> read, int rate=-1, int periodSize=-1, Thread& thread=mainThread):
	AudioOutput(16,rate,periodSize,thread) { read16=read; }
    /// Configures PCM for 32bit output
    /// \note \a read will be called back periodically to request an \a output frame of \a size samples
    /// \note -1 means maximum
    AudioOutput(function<uint(int32* output, uint size)> read, int rate=-1, int periodSize=-1, Thread& thread=mainThread):
    ...
    /// \note -1 means maximum
    AudioOutput(function<uint(int16* output, uint size)> read16, function<uint(int32* output, uint size)> read32, int rate=-1, int periodSize=-1, Thread& thread=mainThread):
...
struct AudioInput : Device, Poll {
    ...
    AudioInput(int sampleBits, int rate, int periodSize, Thread& thread);
    AudioInput(function<uint(const int16* output, uint size)> write, int rate=-1, int periodSize=-1, Thread& thread=mainThread):
    ...
    /// \note read will be called back periodically to provide an \a input frame of \a size samples
    /// \note -1 means maximum
    AudioInput(function<uint(const int32* output, uint size)> write, int rate=-1, int periodSize=-1, Thread& thread=mainThread):
    ...
    /// \note read will be called back periodically to provide an \a input frame of \a size samples
    /// \note -1 means maximum
    AudioInput(function<uint(const int16* output, uint size)> write16, function<uint(const int32* output, uint size)> write32, int rate=-1, int periodSize=-1, Thread& thread=mainThread):
    AudioInput(-1,rate,periodSize,thread) { this->write16=write16; this->write32=write32; }

...
/// Generic Bresenham line stepping
...
struct Surface {
    ...
    void generate() {
        ...
        int2 size; for(int2 p: curve) size=max(size,p);
        ...
        int2 seed=0; int n=0;
        for(uint i: range(curve.size())) line(curve[i],curve[(i+1)%curve.size()], // Rasterize curve on grid
    ...
    void render(const mat4& view) {
        ...
        vec2 last=0;
        for(int2 p: curve) {
            vec2 xy = unskew*vec2(p-size/2);
            xy = (view*vec4(xy.x,xy.y,0,1)).xyz().xy();
            if(last) line(last,xy);
            last = xy;
        }
...
struct MainWindow : Widget {
    Window window __(this,int2(1024,768),"Editor"_);
    MainWindow() { window.localShortcut(Escape).connect(&exit); window.backgroundCenter=window.backgroundColor=0xFF; }
};
...
struct Editor : MainWindow {
    ...
    mat4 view(int2 unused position, int2 size) {
        mat4 view;

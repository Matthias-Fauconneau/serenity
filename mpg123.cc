...
struct mpg123 : AudioFile {
    ...
    void open(const string& path) {
        ...
        if(audioInput.frequency != audioOutput.frequency) {
			resampler = Resampler::instance();
			resampler->setup(audioInput.channels, audioInput.frequency, audioOutput.frequency);
        }
    }
	void close() { mpg123_close(file); mpg123_delete(file); file=0; }
	int position() { return (int)mpg123_tell(file)/audioInput.frequency; }
	int duration() { return (int)mpg123_length(file)/audioInput.frequency; }
	void seek( int time ) { mpg123_seek_frame(file,mpg123_timeframe(file,time),0); }
    void read(int16* output, int outputSize) {
...
AudioFile* AudioFile::instance() { return new mpg123(); }

...
generic struct Lattice {
 ...
 buffer<T> cells {size_t(1+size.y*size.x+size.x+1+ size.z*size.y*size.x + 2*size.y*size.x+2*size.x+2)}; // -1 .. 2 OOB margins
 const mref<T> base = cells.slice(1+size.y*size.x+size.x+1);

...
struct SnapshotView : Widget {
 ...
 shared<Graphics> graphics(vec2 widgetSize) override {
  ...
  if(!target) target = GLFrameBuffer(/*this->size*/int2(widgetSize));
  ...
  //int offset = (target.size.x-widgetSize.x)/2;
  //target.blit(0, int2(widgetSize), int2(offset, 0), int2(target.size.x-offset, target.size.y));
  shared<Graphics> graphics;
  graphics->blits.append(0, widgetSize, target.readback());
  return graphics;
 }

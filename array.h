...
//referencing constructors
    ...
    void reserve(int capacity) {
        ...
        else if(capacity) { T* copy=(T*)malloc((size_t)capacity*sizeof(T)); ::rawCopy(copy,data,size); data=copy; }
    ...
    /// Copies all elements to \a buffer (used by cat)
    ...
    /// remove
    void removeAt(int i) { assert(i>=0 && i<size); for(;i<size-1;i++) rawCopy(at(i),at(i+1)); size--; }
    ...
    /// append array
    void append(array<T>&& a) { int s=size+a.size; reserve(s); rawCopy(end(),a.data,a.size); size=s; }
...
/// Copies all elements in a new array
//FIXME: don't initialize before copy
template<class T> array<T> copy(const array<T>& a) { if(!a.size) return array<T>(); array<T> r(a.size,T()); copy((T*)r.data,a.data,a.size); return r; }

template<class T> T& min(array<T>& a) { T* min=&a.first(); for(T& e: a) if(e<*min) min=&e; return *min; }

...
template <class T> struct Buffer {
    ...
    const T& operator[](uint i) const { debug(if(i>=size)fail();) return data[i]; }
    T& operator[](uint i) { debug(if(i>=size)fail();) return (T&)data[i]; }
};
...
template<class T> struct array {
    int8 tag=-1; /*-1 = heap allocated*/ int8 pad[7]; //8 bytes
    Buffer<T> buffer; //+16
    uint64 cache[5]; //+40 to make sizeof(array) = 64 bytes (allow long[7]/int[15]/short[31]/char[63] to be inlined in same cache line)

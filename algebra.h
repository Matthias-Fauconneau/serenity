...
struct Matrix {
    Matrix(){}
    Matrix(uint n):m(n),n(n){columns.grow(n); columns.mref::clear();}
    Matrix(uint m, uint n):m(m),n(n){columns.grow(n); columns.mref::clear();}
...
inline String str(const Matrix::Element& e) { return str(e.row, e.value); }

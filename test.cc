...
struct Test : Widget {
    int2 gridSize{256,256};
    //int2 gridSize{16,16};
    Grid<Cell> source{gridSize};
    Grid<Cell> target{gridSize};

    Window window{this, gridSize*4/**64*/, "2D Channel"_};
    Test() {
        initialize();
        //step();
        window.show();
    ...
    const real nu = 10e-3/*0e-6*/; // Kinematic viscosity: ν[water] [m²/s]
#if 0
    const real dt = 1./60; // Time step: display rate [s]
    const real dx = sqrt(nu*dt); // Spatial step: δx² α νδt [m]
#else
    const real dx = 4./3700; // Spatial step: cell size [px] / display resolution [px/m] [m]
    ...
    void initialize() {
        log(dx*1000,"mm", dt*1000,"ms", 1./dt,"Hz");
        assert(alpha<1, alpha);
        for(int y: range(gridSize.y)) for(int x: range(gridSize.x)) {
            Cell& cell = source(x,y);
            for(int dy: range(3)) for(int dx: range(3)) cell(dx,dy) = rho / 9;
            //for(int dy: range(3)) for(int dx: range(3)) cell(dx,dy) = 0; cell(1,1) = 1;
        }
        t = 0;
    }
    ...
    void render(int2 position, int2 size) {
        ...
        for(int y: range(gridSize.y)) for(int x: range(gridSize.x)) {
            ...
            for(int dy: range(3)) for(int dx: range(3)) {
                ...
                Text(ftoa(cell.phi[dy*3+dx]/(rho/9),0)).render(p,cellSize/3);
            }
        }*/

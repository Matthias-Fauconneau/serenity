...
inline half8 packs(int4 a, int4 b) { return __builtin_ia32_packssdw128(a,b); }
inline int4 cvtps(float4 a) { return __builtin_ia32_cvtps2dq(a); }
inline float4 load(const float* p) { return *(float4*)p; }
inline float4 load(const float2* p) { return *(float4*)p; }
inline int4 sra(int4 v, int i) { return __builtin_ia32_psradi128(v,i); }
#if DEBUG

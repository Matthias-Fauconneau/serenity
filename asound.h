#pragma once
//TODO: cleanup and inline to alsa.h
...
pcm pcm_open(unsigned int card, unsigned int device,
                     unsigned int flags, struct pcm_config *config);
int pcm_close(pcm& pcm);
int pcm_is_ready(pcm& pcm);

/* Set and get config */
int pcm_get_config(pcm& pcm, struct pcm_config *config);
int pcm_set_config(pcm& pcm, struct pcm_config *config);
...
unsigned int pcm_get_latency(pcm& pcm);
...
int pcm_mmap_write(pcm& pcm, const void *data, unsigned int count);
int pcm_mmap_begin(pcm& pcm, void **areas, unsigned int *offset,
                   unsigned int *frames);
int pcm_mmap_commit(pcm& pcm, unsigned int offset, unsigned int frames);

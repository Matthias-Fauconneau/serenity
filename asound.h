...
struct AudioOutput : Device, Poll {
    ...
    AudioOutput(function<uint(int16* output, uint size)> read, uint rate=44100, uint periodSize=4096, Thread& thread=mainThread()):
    ...
    AudioOutput(function<uint(int32* output, uint size)> read, uint rate=44100, uint periodSize=4096, Thread& thread=mainThread()):

...
void Score::onPath(const array<vec2>& p) {
    if(p.size==2) {
        ...
        if(abs(p[0].y-p[3].y)<1) {
            ties.appendOnce( Line(p[0],p[3]) );
        }
    }
}

void Score::onGlyph(int index, vec2 pos, float size,const string& font, int code) {
    int i=0; for(;i<staffs.size && pos.y>staffs[i];i++) {}
    int duration=-1;
    // Opus is Sibelius default font, Emmentaler is Lilypond default font
    if(font.contains(_("Opus"))||font.contains(_("Emmentaler"))) {
        if(font.endsWith(_("Opus"))||font.contains(_("Emmentaler"))) {
        ...
        if(code==56/*trill head*/) trills << Line(pos,pos);
            ...
            if(m.size==4 && abs(y[0]-y[1])<12 && abs(y[1]-y[2])>100 && abs(y[2]-y[3])<12 ) repeats<<(m[0]+m[1]+m[2]+m[3])/4;
        }
    ...
    } else if(font.contains(_("DUCRGK"))) { //TODO: glyph OCR
...
struct Tie { int li,lx,ly; int ri,rx,ry; Tie():li(0),lx(0),ly(0),ri(0),rx(0),ry(0){}};
void Score::synchronize(array<int> MIDI) {
    staffs << (lastClef.y+96); //add a last split at the bottom of the last page

    //TODO: convert all iterators over keys to iteratos over key, value
    ...
    for(Line tie : ties) {
        ...
        int i=0; for(;i<staffs.size-1 && tie.a.y>staffs[i];i++) {}
        ...
        for(int i : notes.keys) for(int x : notes[i].keys) {
            ...
            for(int y : notes[i][x].keys) {
                if(!repeats.size) { //FIXME
                    ...
                    if(lastSync && sync && notes[i][x].size()<3) for(int j : lastChord.keys) { if(j>=MIDI.size) break;
